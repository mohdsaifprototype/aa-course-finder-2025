...
function showCourseDetail(courseName) {
	...
	document.getElementById("course-detail-title").textContent =
		selectedCourse.name;
	const courseBrouchure = document.getElementById("course-brouchure");
	if (selectedCourse.brouchureLink.length > 0) {
		courseBrouchure.href = selectedCourse.brouchureLink;
		courseBrouchure.toggleAttribute("hidden", false);
	} else {
		courseBrouchure.toggleAttribute("hidden", true);
	}
  ...
  `;
	...
	if (selectedCourse.degree) {
		...
		if (
			selectedCourse.monthlyInstallments_1 <= 0 ||
			selectedCourse.months < 4 ||
			selectedCourse.months - 1 <= 0
		) {
		...
		if (
			selectedCourse.monthlyInstallments_2 <= 0 ||
			selectedCourse.months < 4 ||
			selectedCourse.months - 2 <= 0
		) {
			document.getElementById(
				"course-detail-table4"
			).innerHTML += `<td>N/A</td>`;
		...
		if (
			selectedCourse.monthlyInstallments_3 <= 0 ||
			selectedCourse.months < 4 ||
			selectedCourse.months - 3 <= 0
		) {
			document.getElementById(
				"course-detail-table4"
			).innerHTML += `<td>N/A</td>`;
		...
		if (
			selectedCourse.monthlyInstallments_4 <= 0 ||
			selectedCourse.months < 4 ||
			selectedCourse.months - 4 <= 0
		) {
			document.getElementById(
				"course-detail-table4"
			).innerHTML += `<td>N/A</td>`;
